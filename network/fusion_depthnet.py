...
class FusedDepthNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.aug_depth:
            outputs['extrinsics_aug'] = fusion_dict['extrinsics_aug']
        ...
        if hasattr(self, 'flip_version') and self.mode == 'train':
            if self.flip_version <= 2 or self.flip_version >= 5:
                ...
                for i in range(len(feat_in)):
                    aaa=[]
                    for j in range(len(feat_in[i])):
                        local_feat = feat_in[i][j]
                        if flips[j].item()>0:
                            aaa.append(hflip(local_feat))
                        else:
                            aaa.append(local_feat)
                    aaa = torch.stack(aaa)
                    feat_in_flipped.append(aaa)
                feat_in = feat_in_flipped
        ...
        if hasattr(self, 'flip_version') and self.mode == 'train':
            if self.flip_version <= 2 or self.flip_version >= 5:
                ...
                for scale in self.scales:
                    disps = packed_depth_outputs[('disp',scale)]
                    aaa=[]
                    ...
                    aaa = torch.stack(aaa)
                    packed_depth_outputs[('disp', scale)] = aaa
