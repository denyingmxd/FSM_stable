...
class MonoFSMPoseNet(nn.Module):
    """Pose network """

    def __init__(self, vfg):
    ...
    def forward(self, inputs, frame_ids, cam):
        ...
        image = inputs['color_aug', frame_ids[0], 0][:, cam, ...]
        context =inputs['color_aug', frame_ids[1], 0][:, cam, ...]

        # assert (len(context) == self.nb_ref_imgs)

        input = torch.cat([image,context], 1)
        ...
        pose = 0.01 * pose.view(-1, self.nb_ref_imgs,1, 6)
