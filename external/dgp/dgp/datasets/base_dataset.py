...
class DatasetMetadata:
    ...
    def metadata(self):
        ...
        if cpu_count()>1:
            ...
            stats = np.array([DatasetMetadata._get_scene_container_count_mean_stddev(f) for f in self.scenes])
            counts, means, stddevs = stats[:, 0].reshape(-1, 1), stats[:, 1:4], stats[:, 4:]
...
class BaseDataset:
    ...
    def _build_calibration_table(scene_containers):
        ...
        if cpu_count()>1:
            ...
            all_calibration_tables = [BaseDataset._get_scene_calibration_table(f) for f in calibration_files]
    ...
    def _build_datum_index(self):
        ...
        if cpu_count()>1:
            ...
            datum_index = [BaseDataset._datum_index_for_scene(f) for f in  self.scenes]
    ...
    def _build_annotation_index(self):
        ...
        if cpu_count()>1:
            ...
            annotation_index = [BaseDataset._datum_index_for_scene(f) for f in self.scenes]
    ...
    def list_datum_names_available_in_all_scenes(self):
        ...
        if cpu_count()>1:
            ...
            available_datum_names = set([BaseDataset._datum_names_for_scene(f) for f in self.scenes])
